// Seed script: loads static projects and services into Firestore
...
async function seedCollection(name, items) {
  ...
  if (!existing.empty) {
    console.log(`⏭  "${name}" ya tiene ${existing.size} documentos — omitido para no duplicar.`)
    return
  }

  for (const item of items) {
    const payload = name === 'projects'
      ? { ...item, createdAt: serverTimestamp() }
      : item
    await addDoc(col, payload)
    console.log(`  ✓ ${item.title}`)
  }
  console.log(`✅ "${name}" cargado con ${items.length} items.\n`)
}
...
async function main() {
  ...
  console.log('✨ Seed completo. Puedes cerrar este proceso.\n')
  process.exit(0)
}
